...
class Signature(object):
    ...
    def _create_canonical_request(self):
        payload_hash = hashlib.sha256(
            self.request.get_payload_string().encode("utf-8")
        ).hexdigest()
        return """{method}
{endpoint}
{query_string}
...
{canonical_request_hash}""".format(
            dci_algorithm=self.request.algorithm,
    ...
    def _sign(self, secret):
        ...
        signing_key = hmac.new(
            secret.encode("utf-8"), self.dci_date_str.encode("utf-8"), hashlib.sha256
        ).digest()
        return hmac.new(
    ...
    def _equals(client_signature, header_signature):
        return hmac.compare_digest(
            client_signature.encode("utf-8"), header_signature.encode("utf-8")
        )

    def _update_dci_dates(self, dci_datetime_str):
        dci_datetime = datetime.datetime.strptime(
            dci_datetime_str, self.dci_datetime_format
        )
