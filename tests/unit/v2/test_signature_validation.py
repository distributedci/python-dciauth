...
def test_signature_is_expired():
    ...
    now = datetime(2017, 12, 16, 11, 19, 30)
    assert is_expired(headers, now)
...
def test_signature_is_not_expired():
    headers = {"X-Amz-Date": "20171215T111929Z"}
    height_hour_after = datetime(2017, 12, 15, 19, 19, 29)
    assert is_expired(headers, height_hour_after) is False


def test_signature_is_not_expired_empty_header():
