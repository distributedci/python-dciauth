...
content = open(nrt_json, "rb").read()
headers = generate_headers(
    {"method": "POST", "host": "127.0.0.1:5000", "endpoint": "/api/v1/files", "data": content.decode('utf-8')},
    {"access_key": "remoteci/client_id", "secret_key": "secret"},
)
r = requests.post("http://127.0.0.1:5000/api/v1/files", headers=headers, data=content)
assert r.status_code == 200
assert r.json()["content"] == content

content = json.dumps({"a": 1})
headers = generate_headers(
    {"method": "POST", "host": "127.0.0.1:5000", "endpoint": "/api/v1/files", "data": json.dumps(content)},
    {"access_key": "remoteci/client_id", "secret_key": "secret"},
)
r = requests.post("http://127.0.0.1:5000/api/v1/files", headers=headers, data=content)
assert r.status_code == 200
